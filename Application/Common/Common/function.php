...
function add_water($path){
    ...
    $text_water_word = D("config")->where(array("name"=>"TEXT_WATER_WORD"))->select();
    $text_water_word = $text_water_word[0];
    ...
    if(C('WATER_TYPE')==1){
        // 添加文字水印
        $image->open($path)->text($text_water_word['value'],C('TEXT_WATER_TTF_PTH'),C('TEXT_WATER_FONT_SIZE'),C('TEXT_WATER_COLOR'),C('TEXT_WATER_LOCATE'),0,C('TEXT_WATER_ANGLE'))->save($path);
    }elseif(C('WATER_TYPE')==2){
        // 添加图片水印
        $image->open($path)->water($text_water_word['value'],C('IMAGE_WATER_LOCATE'),C('IMAGE_WATER_ALPHA'))->save($path);
